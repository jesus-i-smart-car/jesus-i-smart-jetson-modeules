...
#define STEER_NEUTRAL_ANGLE 95

#define RAD2DEG(x) ((x)*180./M_PI)
#define DEG2RAD(x) ((x)/180.*M_PI)

#define WayPoints_NO 4
...
void init_vision_region(void)
{
	Vision_Region[0].top    =  6.8;
        Vision_Region[0].bottom =  0.6;
        Vision_Region[0].left   =  0.7;
...
{
    my_waypoints_list[0].y = 0.3;
    my_waypoints_list[0].x = 0.3;
  
    my_waypoints_list[1].y =  -0.5;
    my_waypoints_list[1].x =  -0.5;

    my_waypoints_list[2].y = 0.1;
    my_waypoints_list[2].x = -0.6;
    
    my_waypoints_list[3].y = -1.0;
    my_waypoints_list[3].x = -1.0;
    /*
...
int main(int argc, char **argv)
{
  ...
  {	
    ...
    if((wp_go_id==6)&&(check_car_waypoint_region()==0))  // 바뀌는지점의마지막포인트가3이고바뀌는지점이네비게이션0지역
    {
        c_speed.data= 65;
    }
	// publish topics	
